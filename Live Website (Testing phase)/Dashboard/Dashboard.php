                        ...
                        <?php foreach ($rows as $row): ?>
                            ...
                            <div class="modal fade" id="<?= htmlspecialchars($row['game_id'])?>" tabindex="-1" aria-labelledby="<?= htmlspecialchars($row['game_id'])?>label" aria-hidden="true">
                                <div class="modal-dialog">
                                    <form class="modal-content" method="POST" action="../validation/process_transaction.php">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="first-step">
                                                ...
                                                <div class="input-group mb-3">
                                                    <input type="text" name="account_id" class="form-control" placeholder="Input UID" aria-label="Text input with dropdown button">
                                            ...
                                            <div class="second-step mb-3">
                                                ...
                                                <div class="products">
                                                    <div class="row g-3">
                                                        <!-- Join Table and get games currencies -->
                                                        <?php 
                                                            $query = "SELECT products.product_id, games.currency_name, products.amount, products.price FROM games LEFT JOIN products ON games.game_id = products.game_id WHERE games.game_id = " . $row['game_id'] . ";";
                                                            $queryresult = $conn->query($query);

                                                            $currencies = [];
                                                            if ($queryresult->num_rows > 0) {
                                                                while($hehe = $queryresult->fetch_assoc()) {
                                                                    $currencies[] = $hehe;
                                                                }
                                                            } else {
                                                                echo "No results found.";
                                                        ...
                                                        <?php foreach ($currencies as $curr): ?>
                                                                ...
                                                                <div class="col-6 product">
                                                                    <div class="d-flex border rounded p-2 w-100" onclick="select(this)">
                                                                        ...
                                                                        <?php $purchase_date = date('Y-m-d'); ?>
                                                                        <input type="text" name="product_id" value="<?= $curr['product_id'] ?>" class="d-none">
                                                                        <input type="text" name="game_id" value="<?= $row['game_id'] ?>" class="d-none">
                                                                        <input type="text" name="user_id" value="<?= $_SESSION['user_id'] ?>" class="d-none">
                                                                        <input type="text" name="purchase_date" value="<?= $purchase_date ?>" class="d-none">

                                                                        <img src=<?= $currencyimg ?> style="width: 50px; height: 50px;">
                                                                        <div class="mx-2">
                                                                            <span style="font-weight: 700;" class="amount_value"><?= htmlspecialchars($curr['amount'])?> <?= htmlspecialchars($curr['currency_name'])?></span>
                                                                            <p class="text-secondary m-0"><?= htmlspecialchars($curr['price'])?>php</p>
                                                                        </div>
                                                                    </div>
                                                                </div>
